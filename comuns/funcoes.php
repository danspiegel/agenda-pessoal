...
function paginaAtual($pagina)
{
    ...
    if( ( isset($_GET['secao']) && $_GET['secao']==$pagina ) || ( !isset($_GET['secao']) && $pagina=='home' ) )
    {
...
// Função que recebe o id do contato e pesquisa na pasta de imagem se ele tem foto. Retorno: Caminho para a imagem (string).
...
function uploadFoto($arquivo, $novo_nome) {
    ...
    // Array com os tipos aceitos para esse upload
    ...
    // Verificação de erros no upload do arquivo
    if(!empty($arquivo['error']))
    {
        switch($arquivo['error'])
        {
            ...
            case '3':
                ...
                break;
            case '4':
                $resultado = 'Nenhum arquivo para adicionar.';
                break;
            case '6':
